...
class Solver(object):
    ...
    def _reset(self):
        ...
        if self.args.continue_from:
            ...
            # checkpoint = torch.load('/home/vgc/users/zhaopeng/USEV/pretrain_networks/sdr-av-dprnn-pretrain-noise.pt', map_location='cpu')
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.args.epochs+1):
            ...
            self.model.train()
            start = time.time()
            tr_loss,tr_sisnri = self._run_one_epoch(data_loader = self.train_data,state='train')
            tr_loss = self._reduce_tensor(tr_loss)

            if self.print: print('Train Summary | End of Epoch {0} | Time {1:.2f}s | '
                      'Train Loss {2:.3f}| avg_sisnri {3:.4f}'.format(
                        epoch, time.time() - start, tr_loss ,tr_sisnri))

            # Validation
            self.model.eval()
            start = time.time()
            with torch.no_grad():
                val_loss,val_sisnri = self._run_one_epoch(data_loader = self.validation_data, state='val')
                reduced_val_loss = self._reduce_tensor(val_loss)

            if self.print: print('Valid Summary | End of Epoch {0} | Time {1:.2f}s | '
                      'Valid Loss {2:.3f}| avg_sisnri {3:.4f}'.format(
                          epoch, time.time() - start, reduced_val_loss, val_sisnri))

            # test
            self.model.eval()
            start = time.time()
            with torch.no_grad():
                test_loss,test_sisnri = self._run_one_epoch(data_loader = self.test_data, state='test')
                test_loss = self._reduce_tensor(test_loss)


            if self.print: print('Test Summary | End of Epoch {0} | Time {1:.2f}s | '
                      'Test Loss {2:.3f}| avg_sisnri {3:.4f}'.format(
                          epoch, time.time() - start, test_loss,test_sisnri))
    ...
    def _run_one_epoch(self, data_loader, state='train'):
        ...
        for i, (a_mix, a_tgt, v_tgt) in enumerate(data_loader):
            ...
            # print(loss.item())
            mix_snr = cal_SISNR(a_tgt, a_mix)
            max_snr = cal_SISNR(a_tgt, a_tgt_est)
            
            sisnri = max_snr - mix_snr

            avg_sisnri += torch.mean(sisnri)
